...
- **`services/slaMonitor.ts`** runs a 60-second in-process sweep (`setInterval`, started from `server.ts` — no new infra, matches the existing single-process architecture):
  - `sweepUnansweredOrders` — orders past their accept deadline still `placed` get auto-rejected (`transitionOrderStatus(..., { auto: true })`), the pickup slot is released, the student is notified, then sent a **tappable list of nearby alternative stalls** (same `stall:<id>` scheme as Browse Stalls — tapping one primes `browseFlow` to continue shopping there, reusing the existing flow rather than dead-ending). `autoRejected` is a distinct flag from an owner-initiated reject, so it's never counted against the owner. **(2026-08-09 fix)** The query also now matches `acceptDeadline: null` and any `placed` order older than 24h regardless of its deadline — a real order was found stuck in "placed" for 5+ days with a null `acceptDeadline` (`placeOrder()` always sets one, so this was likely from data created outside that flow), invisible to the original `acceptDeadline: {lte: now}` filter since Postgres's `lte` never matches NULL. It sat in the owner's "needs attention" queue until manually rejected, sending a confusing "your order was rejected" WhatsApp message to a student who'd ordered almost a week earlier. Regression-tested in `tests/integration/slaFlow.test.ts`.
...
- **Two real bugs found and fixed by this pass** (not pre-existing — both were introduced earlier this session by the Offers Engine and this test-fixture code, and are now fixed; not left as known issues):
  ...
  3. **(2026-08-09) `createPickupSlot`'s local-timezone-dependent slot construction**: built `slotDate`/`startTime`/`endTime` via local `Date.setHours` (matching, unknowingly, the exact same bug later found and fixed in `slotGenerator.ts` — see §4.7). On a dev machine running in IST, this makes `slotDate` land on the *previous* UTC calendar date for roughly a third of the day, since IST is UTC+5:30 — invisible as long as the whole test run happened to execute before the boundary, but a grace-period test started failing partway through this same session once real wall-clock time crossed it. Fixed by rebuilding both helpers with pure `Date.UTC` arithmetic (no local-timezone dependency at all) — `createPickupSlot`'s `hour` option is now UTC, not IST; the one test that cared about "the current hour" (`slaFlow.test.ts`'s grace-period boundary test) was updated to use `getUTCHours()` accordingly.
